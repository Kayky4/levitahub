...
import { Band, BandMember, UserRole, BandBilling, Playlist, RegencySession } from '../../services/types';
...
const BandDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // 1. Fetch Static Data
        const [bandData, membersData, billingData, nextEventData, songsData] = await Promise.all([
        ...
        setSongsCount(songsData.length);
  ...
  const isPlus = billing?.subscriptionStatus === 'active' || billing?.subscriptionStatus === 'trialing';
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-midnight-900 pb-20 transition-colors duration-300">
      ...
      {bandId && <RestrictionBanner bandId={bandId} status={billing?.subscriptionStatus} />}
