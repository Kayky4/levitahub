...
const RegencyController: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        setStatus('ready');
        getBandSongs(bandId, null, 100).then(data => setAllBandSongs(data.songs));
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      if (containerRef.current && scrollState.isPlaying) {
  ...
  // 5. Sync Scroll Position
  useEffect(() => {
    if (!bandId || !scrollState.isPlaying) return;

    const interval = setInterval(() => {
      if (containerRef.current) {
        syncScrollPosition(bandId, containerRef.current.scrollTop);
      }
    }, 2000);

    return () => clearInterval(interval);
  }, [bandId, scrollState.isPlaying]);
