...
const RegencyViewer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      if (containerRef.current && session?.scrollState?.isPlaying && followLeader && !userHasScrolled) {
        const safeIndex = Math.min(Math.max(session.scrollState.speed, 0), SPEED_LEVELS.length - 1);
        const speedPxPer16ms = SPEED_LEVELS[safeIndex];
        const pxPerMs = speedPxPer16ms / 16;

        const move = pxPerMs * deltaTime;
      ...
      if (session?.scrollState?.isPlaying) {
  ...
  // 6. Soft Catch-up
  useEffect(() => {
    if (!containerRef.current || !session?.scrollState?.scrollTop || !followLeader || userHasScrolled) return;

    const currentScroll = containerRef.current.scrollTop;
    const targetScroll = session.scrollState.scrollTop;
    const diff = Math.abs(currentScroll - targetScroll);
    const viewportHeight = containerRef.current.clientHeight;

    // If difference is > 5% of viewport, adjust
    if (diff > viewportHeight * 0.05) {
      containerRef.current.scrollTo({
        top: targetScroll,
        behavior: 'smooth'
      });
    }
  }, [session?.scrollState?.scrollTop, followLeader, userHasScrolled]);
