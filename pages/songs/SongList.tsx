...
const SongList: React.FC = () => {
  ...
  const loadMore = async () => {
    if (!bandId || !lastDoc || loadingMore) return;
    setLoadingMore(true);
    try {
      const nextBatch = await getBandSongs(bandId, lastDoc, 20);
      setSongs(prev => [...prev, ...nextBatch.songs]);
      setLastDoc(nextBatch.lastDoc);
      setHasMore(!!nextBatch.lastDoc);
    } catch (error) {
