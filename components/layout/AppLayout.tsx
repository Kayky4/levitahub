  ...
  return (
    <div className="min-h-screen min-h-[100dvh] bg-gray-50 dark:bg-midnight-900 text-gray-900 dark:text-slate-200 font-sans selection:bg-indigo-500/30 transition-colors duration-300 flex flex-col">
      ...
      <main className={`flex-1 relative ${!hideNavbar ? 'pt-16' : ''} ${currentBandId ? 'pb-32 md:pb-0' : ''}`}>
        {/* Subscription Banner */}
        <SubscriptionBanner bandId={currentBandId} />
        {children}
      </main>
...
// Internal Component for Banner
const SubscriptionBanner = ({ bandId }: { bandId: string | null }) => {
  const [status, setStatus] = useState<'active' | 'trial' | 'expired' | 'blocked' | null>(null);
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        const { getBandDetails } = await import('../../services/bands');
        const { getSubscriptionState } = await import('../../services/billing');

        const band = await getBandDetails(bandId);
        if (band && isMounted) {
          const state = getSubscriptionState(band);
          setStatus(state.status as any);
          setDaysRemaining(state.daysRemaining);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  if (status === 'trial' && daysRemaining > 5) return null; // Don't annoy if plenty of trial left

  const isWarning = status === 'trial' || status === 'expired';
  const isCritical = status === 'blocked';

  return (
    <div className={`
      w-full px-4 py-2 text-center text-xs font-bold uppercase tracking-wider
      ${isCritical ? 'bg-red-600 text-white' : 'bg-indigo-600 text-white'}
    `}>
      {status === 'trial' && `Período de teste: ${daysRemaining} dias restantes.`}
      {status === 'expired' && `Assinatura vencida. Renove para evitar bloqueio.`}
      {status === 'blocked' && `Banda bloqueada. Entre em contato para regularizar.`}
    </div>
  );
};
