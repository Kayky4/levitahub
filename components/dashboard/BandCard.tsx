...
const BandCard: React.FC<BandCardProps> = ({ id, name: initialName, role, index }) => {
  ...
  // Default gradient if fetch fails or while loading
  const defaultGradient = GRADIENTS[id.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % GRADIENTS.length];
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        // Fetch Band Details (for Theme & Logo) AND Next Event in parallel
        const [bandData, event] = await Promise.all([
          import('../../services/bands').then(mod => mod.getBandDetails(id)),
          getNextBandEvent(id)
        ]);
        ...
        if (isMounted) {
          if (bandData) {
            setBandDetails({
              name: bandData.name,
              themeColor: bandData.themeColor || defaultGradient,
              logoSymbol: bandData.logoSymbol || bandData.name.substring(0, 2).toUpperCase()
            });
          }
          setNextEvent(event);
        }
  ...
  // Use fetched details or fallbacks
  const displayName = bandDetails?.name || initialName;
  const displayTheme = bandDetails?.themeColor || defaultGradient;
  const displaySymbol = bandDetails?.logoSymbol || initialName.charAt(0).toUpperCase();
  ...
  return (
    ...
    >
      ...
      <div className={`h-24 bg-gradient-to-r ${displayTheme} relative p-6 transition-colors duration-500`}>
      ...
      <div className="px-6 pb-6 pt-2 flex-1 flex flex-col justify-between relative z-20">
        <div>
          <div className="-mt-10 mb-3">
            <div className="h-16 w-16 bg-white dark:bg-midnight-700 rounded-2xl shadow-lg flex items-center justify-center text-3xl p-1 border-4 border-white dark:border-midnight-800 transition-colors">
              <span className={`font-black bg-gradient-to-br ${displayTheme} bg-clip-text text-transparent uppercase`}>
                {displaySymbol}
              </span>
            </div>
          </div>
